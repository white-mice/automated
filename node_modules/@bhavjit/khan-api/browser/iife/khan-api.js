...
var KhanAPI = (() => {
  ...
  // .yarn/cache/cross-fetch-npm-3.1.5-e414995db9-f6b8c6ee3e.zip/node_modules/cross-fetch/dist/browser-ponyfill.js
...
`
    ...
    transformSchema(schema) {
      return {
        ...
        qualarooID:
          ...
          isQualarooID(schema.qualarooId)
  ...
  function programIDtoKey(id) {
    ...
    const encoded = id
      ...
      .match(/.{1,7}(?=(.{7})*$)/g)
      .map((r) =>
        String.fromCharCode(parseInt(r.length === 7 ? `1${r}` : r, 2))
      )
      .reverse()
      .join('')
    let base64 = btoa(
...
Scratchpad${encoded}\f`
  ...
  var _Program = class extends Wrapper {
    ...
    transformSchema(schema) {
      return {
        ...
        updated: (() => {
          if (!schema.revision?.created) return void 0
  ...
  function stripCookies(cookies) {
    return cookies
      .replace(
        /[a-zA-Z]+, \d\d [A-Za-z0-9]+ \d{4} \d\d:\d\d:\d\d [a-zA-Z]+/g,
        ''
      )
  ...
  var _Client = class {
    ...
    async getUser(identifier) {
      ...
      if (user.kaid) {
        if (
          user.username &&
          !__privateGet(this, _cachedKaids).has(user.username)
        )
    ...
    async *getUserPrograms(identifier, sort = 'TOP' /* TOP */, limit = 40) {
      ...
      const getUserProgramsPage = async (cursor2) => {
        ...
        if (!json.data.user) throw new Error('User not found')
        if (
          !json.data.user.programs.complete &&
    ...
    async getAllProgramMessages(
      ...
      const messages = []
      for await (const page of this.getProgramMessages(
        type,
        identifier,
        sort
      )) {
